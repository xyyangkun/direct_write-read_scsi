         ...
         int bs, const struct flags_t * ofp, int * diop)
{
    ...
    //printf("debug:ofp->cdbsz:%d,blocks:%d,to_block:%d,bs:%d\n",ofp->cdbsz,blocks,to_block,bs);
...
int hd_write(int fd, long long  seek, unsigned int blk_num,unsigned char *write_buf,unsigned int buf_size)
{

	int dio_tmp=0;
    int blks_read=0;
    int blk_sz=BLOCKSIZE;
    int ret;

    if(blk_num*BLOCKSIZE < buf_size)
    {
    	printf("error: the write buf size is small\n");
    	return -1;
    }

	unsigned int i=0;
	if(blk_num>=MAXREADBLOCK)//yk change 20130731  >  ->  =>  不然在32 64..会出错
	{
		for(i=0; i<blk_num/MAXREADBLOCK; i++)
		{
		    ret=sg_write(fd, write_buf+i*MAXREADBLOCK*BLOCKSIZE, MAXREADBLOCK,\
		    		seek+i*MAXREADBLOCK, blk_sz, &oflag, &dio_tmp);
		    if(ret!=0)
		    {
	...
	if(blk_num%MAXREADBLOCK==0)  //yk add 20130731
		return 0;
	/*如果blk_num> MAXREADBLOCK,则i>0,因为上面的那个循环执行了。
	 如果 blk_num不是MAXREADBLOCK的整数倍，读剩余部分*/
    ret=sg_write(fd, write_buf+i*MAXREADBLOCK*BLOCKSIZE, blk_num%MAXREADBLOCK,\
    		seek+i*MAXREADBLOCK, blk_sz, &oflag, &dio_tmp);
    if(ret!=0)
    {
    	printf("write date buf error ,plear resove the probram,and i will exit()\n");
    ...
    //if(dio_tmp!=blk_num%MAXREADBLOCK)printf("maybe someerror write ,dio_tmp:%d\n",dio_tmp);
return 0;
 ...
 * 返回值：读的字节数， -1失败
 ********************************************************************************/
int hd_read(int fd, long long  seek, unsigned int blk_num,unsigned char *read_buf,unsigned int buf_size)
{
	int dio_tmp=0;
    int blks_read=0;
    int blk_sz=BLOCKSIZE;
    ...
    if(blk_num*BLOCKSIZE < buf_size)
    {
    	printf("error: the read buf size is small\n");
    	return -1;
    }

	unsigned int i=0;
	if(blk_num>=MAXREADBLOCK)  //yk change 20130709
	{
		for(i=0; i<blk_num/MAXREADBLOCK; i++)
		{
			ret=sg_read(fd, read_buf+i*MAXREADBLOCK*BLOCKSIZE, MAXREADBLOCK,\
					seek+i*MAXREADBLOCK,blk_sz, &iflag, &dio_tmp, &blks_read);
			if(ret!=0)
			{
				printf("hd read block error1,and i will exit() \n");
				return -1;
			}
			if(blks_read!=MAXREADBLOCK)printf("maybe some read error ,blks_read:%d\n",blks_read);
		}
	}
	if(blk_num%MAXREADBLOCK==0)  //yk add 20130709
		return 0;
	/*如果blk_num> MAXREADBLOCK,则i>0,因为上面的那个循环执行了。
	 如果 blk_num不是MAXREADBLOCK的整数倍，读剩余部分*/
	//printf("DDD%d,i:%d\n",blk_num%MAXREADBLOCK,i);
	ret=sg_read(fd, read_buf+i*MAXREADBLOCK*BLOCKSIZE,blk_num%MAXREADBLOCK,\
			seek+i*MAXREADBLOCK,blk_sz, &iflag, &dio_tmp, &blks_read);
	if(ret!=0)
	{
		printf("hd read block error2,and i will exit() \n");
		return -1;
	}
	if(blks_read!=blk_num%MAXREADBLOCK)printf("maybe some read error ,blks_read:%d\n",blks_read);
return 0;
...
int init_sda()
{
	...
	/*必要的初始化*/
	oflag.sgio=1; /*sgio*/
	oflag.cdbsz= MAX_SCSI_CDBSZ /*yk change DEF_SCSI_CDBSZ*/  ;
	bpt = DEF_BLOCKS_PER_2048TRANSFER;
	outfd = open_of("/dev/sda", seek, bpt, &oflag, &out_type, verbose);
	if (outfd < -1)
		return -outfd;
	int dio_tmp = oflag.dio;
	iflag.retries=0;
	iflag.sgio=1;
	iflag.cdbsz= MAX_SCSI_CDBSZ /*yk change DEF_SCSI_CDBSZ*/  ;
	blk_sz=512;
	return 0;
}
